...
class Auth():
    ...
    def generate_token(user_id):
        ...
        try:
            ...
            return jwt.encode(
                payload,
                os.getenv('JWT_SECRET_KEY', 'hhgaghhgsdhdhdd'),
                'HS256'
            ).decode("utf-8")
    ...
    def decode_token(token):
        ...
        re = {'data': {}, 'error': {}}
        try:
            payload = jwt.decode(token, os.getenv('JWT_SECRET_KEY', 'hhgaghhgsdhdhdd'))
            re['data'] = {'user_id': payload['sub']}
            return re
        except jwt.ExpiredSignatureError as e1:
            re['error'] = {
                'message': 'token expired, please login again' + str(e1)}
            return re
